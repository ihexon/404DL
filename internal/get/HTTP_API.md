...
- 用户点击搜索结果的 Download 后，Web UI 调用 `POST /api/tasks` 创建持久下载任务，再调用 `PUT /api/tasks/{id}/continue` 继续任务；只有这一步才会进入 anacrolix/torrent。命名和方法对齐 Gopeed 的 continue task 操作。
