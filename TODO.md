...
- Move runtime updates from polling to SSE:
  - Keep `GET /api/torrents/{id}/runtime` for initial/full snapshot fetches.
  - Add `GET /api/torrents/{id}/runtime/stream` for live snapshots.
...
- Rewrite `README.md` so it describes the project, CLI, API, httpfs UI, runtime diagnostics, and configuration.
